...
|---------|------|-------------|
| 1.0.0   | ≥ 18 | Browse and search GDPR with citations; optional LLM summaries. |
...
- **Credible sources** — [GDPR-Info](https://gdpr-info.eu/) and [EUR-Lex – Regulation (EU) 2016/679](https://eur-lex.europa.eu/eli/reg/2016/679/oj/eng). All links point to these; no third-party legal content.

### 3.4 Optional LLM summaries

- **Summary box** — When at least one LLM API key is set, the right-hand Summary can be generated by an LLM instructed to use **only** the provided regulation excerpts (strict anti-hallucination).
...
- Tab switching: `data-view="browse"` | `data-view="ask"`; views shown/hidden and `aria-selected` updated.
...
- **Credible sources only** — Answer text is limited to the regulation content from EUR-Lex; LLM summaries are constrained to that same text to avoid hallucination.
- **Attribution** — Footer and UI attribute content to gdpr-info.eu and EUR-Lex; links must remain so users can check originals.

---

## 6. Tech guidelines

- **Node.js** — Required (engine `>=18`). Uses native `fetch` for LLM and scraper HTTP.
...
- **Data directory** — `data/` must be writable by the process for refresh (writes `gdpr-content.json`). `gdpr-structure.json` must exist for the scraper to run.
...
- **Accessibility** — Tabs and panels use `role="tablist"`, `role="tab"`, `role="tabpanel"`, `aria-selected`; buttons have `aria-label` where needed.
...
| **Scheduling** | node-cron (daily refresh) |
| **Scraping** | axios, cheerio (or built-in https + regex fallback) |
...
| **Data** | JSON files in `data/` (gdpr-structure.json, gdpr-content.json) |
...
```
gdpr-qa-platform/
├── server.js              # Express app: API routes, LLM summarize, refresh, cron, static serve
...
└── public/
    ├── index.html         # Single-page UI: header, tabs (Browse / Ask), footer
    ├── styles.css         # Layout, typography, components, responsive rules
    └── app.js             # API client, Browse (recitals, chapters, detail, PDF), Ask (search, summarize, View in app, Back to question)
```
...
| File | Responsibility |
|------|----------------|
| **server.js** | `loadContent`, REST API (meta, categories, chapters, articles, recitals, ask, summarize, refresh), `simpleSearch`, LLM providers (OpenAI, Anthropic, Gemini, Groq, Mistral, OpenRouter), extractive `buildSummaryFromExcerpts`, cron, static serve, SPA fallback. |
...
| **public/index.html** | Markup: header (logo, lastRefreshed, Refresh), tabs (Browse, Ask), Browse (sidebar, placeholder, recitals list, chapters list, detail + citations, Back / Back to question / Export PDF), Ask (query input, askResults grid: overallSummary + resultsList, askSummaryPanel), footer, html2pdf.js script, app.js. |
| **public/app.js** | `get`/`post`, DOM refs, setMeta/loadMeta, tab switching, showSection, loadRecitals/loadChapters, openRecital/openArticle, citation links, Export PDF (html2pdf), app-goto-doc handler, escapeHtml/formatRecitalRefs, buildClientSummary, doAsk (clear → ask → render results + summarize). |
| **public/styles.css** | Variables, reset, layout (header, tabs, main), sidebar, content-panel, detail-actions, placeholder, doc-grid, detail-view, qa-* and summary-in-plain-language, ask-results-grid, result-card, responsive and print. |
...
| Method | Path | Purpose |
|--------|------|---------|
| GET | `/api/meta` | Last refreshed timestamp, source list |
...
5. (Optional) Set one or more LLM API keys (see `.env.example`) for AI-generated summaries grounded in the regulation text.

---

## 12. License and disclaimer

This project is for **reference only**. The GDPR text is from the official EU sources. Always verify against [gdpr-info.eu](https://gdpr-info.eu/) and [EUR-Lex (Regulation (EU) 2016/679)](https://eur-lex.europa.eu/eli/reg/2016/679/oj/eng). The maintainers do not provide legal advice.
