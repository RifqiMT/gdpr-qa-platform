...
/** Display order for regulation selector and browse welcome cards. */
const REGULATION_DISPLAY_ORDER = ['gdpr', 'data-act', 'ai-act'];
...
function listRegulations() {
  return REGULATION_DISPLAY_ORDER.map((id) => REGULATIONS[id]).filter(Boolean).map((r) => ({
    id: r.id,
