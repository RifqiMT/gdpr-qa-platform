/**
 * EU AI Act content fetcher — articles and recitals from https://ai-act-law.eu/
...
function capBodyText(body) {
  const raw = process.env.AI_ACT_MAX_ARTICLE_CHARS || process.env.GDPR_MAX_ARTICLE_CHARS;
...
function extractAiActArticleFromText(n, text, h1Parts, reg) {
  const lines = cleanLines(text);
  let title = (h1Parts.titleEl || '').replace(/\*\s*$/, '').trim();
  if (!title) {
    const m = (h1Parts.full || '').match(new RegExp(`Art\\.\\s*${n}\\s*AI Act\\s*(.+)$`, 'i'));
...
function extractAiActRecitalFromText(n, text, h1Parts, reg) {
  const lines = cleanLines(text);
  let title = (h1Parts.titleEl || '').replace(/\*\s*$/, '').trim();
  if (!title) {
...
async function fetchAiActLawDataset(reg) {
  if (!cheerio) throw new Error('cheerio not available');
  const maxConcurrency = Math.max(1, parseInt(process.env.AI_ACT_INFO_CONCURRENCY || process.env.GDPR_INFO_CONCURRENCY || '6', 10));
  const tasks = [];
  ...
  async function worker() {
    while (idx < tasks.length) {
      ...
      try {
        ...
        if (t.kind === 'article') {
          results.articles.push(extractAiActArticleFromText(t.n, plain, h1Parts, reg));
        } else {
          results.recitals.push(extractAiActRecitalFromText(t.n, plain, h1Parts, reg));
        }
      } catch (_) {
        /* skip failed page */
      }
...
async function run() {
  ...
  const minArt = Math.max(1, parseInt(process.env.MIN_AI_ACT_ARTICLES || String(reg.minArticles), 10));
  const minRec = Math.max(1, parseInt(process.env.MIN_AI_ACT_RECITALS || String(reg.minRecitals), 10));

  console.log(`ETL: fetching EU AI Act from ${reg.infoBaseUrl} …`);
  const out = await fetchAiActLawDataset(reg);
  const arts = enrichArticlesWithChapter(out.articles, reg.chapterRanges);
  if (arts.length < minArt || out.recitals.length < minRec) {
    throw new Error(
      `AI Act ETL incomplete: articles ${arts.length}/${minArt}, recitals ${out.recitals.length}/${minRec}`
    );
  }
  ...
  const oldHash = existing.meta?.datasetHash || computeDatasetHash(existing.recitals || [], existing.articles || []);
  const forceWrite =
    process.env.AI_ACT_FORCE_CORPUS_WRITE === '1' || process.env.GDPR_FORCE_CORPUS_WRITE === '1';
